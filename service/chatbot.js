...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!token) {
    ...
    appendErrorMessage(
      "Bạn cần đăng nhập để sử dụng dịch vụ này. <a href='../pages/login.html'>Đăng nhập ngay</a>"
    );
    return;
  }
...
async function sendMessage() {
  ...
  // Kiểm tra đăng nhập lại (phòng trường hợp token bị xóa khi đang mở tab)
  const token = localStorage.getItem("token");
  if (!token) {
    appendErrorMessage(
      "Bạn cần đăng nhập để sử dụng dịch vụ này. <a href='../pages/login.html'>Đăng nhập ngay</a>"
...
// Tự động disable chat nếu token bị xóa khi đang mở tab (logout ở tab khác hoặc reload)
